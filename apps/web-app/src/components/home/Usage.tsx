...
function RegisterStep({ active }: { active: boolean }) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-secondary/50 text-xs font-mono text-muted-foreground">
...
function OIDCStep({ active }: { active: boolean }) {
  ...
  const lines: { text: string; color: string; highlight?: boolean }[] = [
    { text: "// oidc.config.js", color: "text-[#526050]" },
    {
      text: "import { OidcClient } from 'oidc-client-ts';",
      color: "text-blue-400",
    },
    { text: " ", color: "" },
    { text: "export const oidcConfig = {", color: "text-[#DDE5D8]" },
    {
    ...
    { text: "  scope: 'openid profile email',", color: "text-[#DDE5D8]" },
    { text: "};", color: "text-[#DDE5D8]" },
  ];
  ...
  return (
    <div className="space-y-3">
      ...
      <div
        className="p-4 rounded-xl border bg-[#0f1109]"
        style={{ borderColor: "var(--color-border)" }}
      >
